...
public final class DataNode {
    ...
    public DataNode(final String dataNode) {
        ...
        // modified start:支持actual-data-nodes“数据源名.数据库.表”格式配置
        if (!dataNode.contains(DELIMITER)) {
            throw new ShardingSphereConfigurationException("Invalid format for actual data nodes: '%s'", dataNode);
        }
        int delimiterCount = Splitter.on(DELIMITER).omitEmptyStrings().splitToList(dataNode).size();
        ...
        if (2 == delimiterCount) {
            ...
            tableName = segments.get(1) + "." + segments.get(2);
        } else {
            throw new ShardingSphereConfigurationException("Invalid format for actual data nodes: '%s'", dataNode);
        }
        // modified end
